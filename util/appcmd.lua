...
local option_actions = {
    ...
    edit = function(ia, where, args)
        ...
        if not found then
            return tools.argError(ia, "what` or `where",
                "Option `" .. (args.where and (args.where .. ".") or "") .. args.what .. "` not found")
        end
...
function endpoints.option(ia, cmd, args, action, action_report)
    ...
    ia:reply(
    action_report ..
    (args.where and (args.where .. ".") or "") ..
    (args.name or args.what) .. "` option in " .. tools.serializeApplicationCommand(cmd), true)
end
    ...
    CLIENT:on("ready", function()
        local appcmd, err = CLIENT:createGuildApplicationCommand(GUILD,
            ...
            :addOption(
                ...
                :addOption(tools.string("description",
                    "Command desciption (will be ignored for non slash commands types)"):setRequired(true))
                :addOption(
            ...
            :addOption(
                ...
                :addOption(
                    ...
                    :addOption(tools.number("min_value",
                        "Minimum value for the option (Only for integer and number types)"))
                    :addOption(tools.number("max_value",
                        "Maximum value for the option (Only for integer and number types)"))
                    :addOption(tools.boolean("autocomplete",
                        "Autocompletion feature (only for string, integer and number types, false by default)"))
                    :addOption(
                ...
                :addOption(
                    ...
                    :addOption(tools.boolean("autocomplete",
                        "Autocompletion feature (only for string, integer and number types, false by default)"))
                    :addOption(tools.string("channel_types",
                        "Channel types allowed to pick separated by space (Only for channel type)"))
                )
                :addOption(
                    tools.subCommand("delete", "Delete option")
                    :addOption(tools.string("id", "ApplicationCommand ID"):setRequired(true):setAutocomplete(true))
                    :addOption(tools.string("what", "Option name"):setRequired(true))
                    :addOption(tools.string("where",
                        "Place where the option is (example: option.create) (root level by default)"))
                )
                :addOption(
